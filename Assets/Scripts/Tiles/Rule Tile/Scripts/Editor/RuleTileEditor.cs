...
namespace UnityEditor
{
	...
	internal class RuleTileEditor : Editor
	{
		...
		internal const float k_DefaultElementHeight = 80f;
		...
		internal static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
		{
			...
			tilingRule.m_Enabled = EditorGUI.Toggle(new Rect(rect.xMin + k_LabelWidth, y - 1, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Enabled);
			y += k_SingleLineHeight;
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Other Neighbors");
			tilingRule.m_OtherNeighbors = EditorGUI.Toggle(new Rect(rect.xMin + k_LabelWidth, y - 1, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_OtherNeighbors);
			y += k_SingleLineHeight;
